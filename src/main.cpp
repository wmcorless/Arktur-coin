...
#if defined(NDEBUG)
#error "Arktur cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zArktur spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zArktur spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zArktur spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if(nHeight >=1 && nHeight <= 500) { //PoW phase
		nSubsidy = 5 * COIN;
    } else if(nHeight >500 && nHeight <= 1000) { //PoW phase
		nSubsidy = 10 * COIN;
    } else if(nHeight >1000 && nHeight <= 1500) { //PoW phase
		nSubsidy = 20 * COIN;
	} else if(nHeight > 1500 && nHeight <= 2000) { //PoS phase
		nSubsidy = 10 * COIN; // "instamine"
    } else if(nHeight > 2000 && nHeight <= 5000) {
		nSubsidy = 130 * COIN;
    } else if(nHeight > 5000 && nHeight <= 8500) { 
		nSubsidy = 170 * COIN;
    } else if(nHeight > 8500 && nHeight <= 12500) { 
		nSubsidy = 210 * COIN;
    } else if(nHeight > 12500 && nHeight <= 21000) { 
		nSubsidy = 250 * COIN;
    } else if(nHeight > 21000 && nHeight <= 35000) { 
		nSubsidy = 125 * COIN;
    } else if(nHeight > 35000 && nHeight <= 100000) { 
		nSubsidy = 75 * COIN;
    } else if(nHeight > 100000 && nHeight <= 1100000) { 
		nSubsidy = 60 * COIN;
    } else if(nHeight > 1100000 && nHeight <= 1500000) { 
		nSubsidy = 45 * COIN;
    } else if(nHeight > 1500000 && nHeight <= 2300000) { 
		nSubsidy = 15 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Arktur
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zArktur with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zArkturSpent: %s\n",
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zArktur spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zArktur serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zArktur mints and %d zArktur spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Arktur: We use certain sporks during IBD, so check to see if they are
