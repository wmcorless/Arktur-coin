...
{
    ZArkturControlDialog* zArkturControl = new ZArkturControlDialog(this);
    zArkturControl->setModel(walletModel);
    zArkturControl->exec();
}

void PrivacyDialog::setZArkturControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzArkturSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzART()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Arktur Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    // use mints from zArktur selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZArkturControlDialog::listSelectedMints.empty()) {
    ...
    // Clear zarktur selector in case it was used
    ZArkturControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zArktur Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Arktur, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zArktur Mint");
