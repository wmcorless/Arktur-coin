...
#include "zarkturcontroldialog.h"
#include "ui_zarkturcontroldialog.h"
...
std::list<std::string> ZArkturControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZArkturControlDialog::listMints;

ZArkturControlDialog::ZArkturControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZArkturControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZArkturControlDialog::~ZArkturControlDialog()
{
    delete ui;
}

void ZArkturControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZArkturControlDialog::updateList()
{
...
void ZArkturControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZArkturControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZArktur_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZArkturControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZArkturControlDialog::GetSelectedMints()
{
...
void ZArkturControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
